    ...
    <b-modal ref="thymodal" id="skillModal" body-class="p-0" class="p-0" :title="currentTab" hide-header hide-footer>
      ...
      <b-card body-class="p-0 rounded-0" class="text-center border-0">
        <b-list-group class="text-left" v-for="skills in skills" v-bind:key="skills.id" v-bind:id="skills.id" v-if="skills.id === currentTab">
          <b-list-group-item v-bind:class="skills.color" class="border-0 rounded-0 text-light" target="_blank" v-bind:href="skills.link"><i class="fal fa-building fa-fw"></i> {{skills.title}} - <em>{{skills.company}}</em></b-list-group-item>
          <b-list-group-item v-bind:class="skills.color" class="border-0 border-dark text-light"><i class="fal fa-calendar-alt fa-fw"></i> {{skills.time}} - {{ $t('today') }}</b-list-group-item>
          <b-list-group-item class="border-0" variant="light"><i class="fal fa-info fa-fw"></i> {{skills.text}}</b-list-group-item>
        </b-list-group>
      </b-card>
...
export default {
  ...
  data() {
    return {
      currentTab: 'seo',
      skills: [
        { title: this.$t('skillSeoTitle'), description: this.$t('skillSeoDescr'), text: this.$t('skillSeoText'), name: 'seo', id: 'seo', icon: 'fal fa-chart-bar', color: 'bg-success', company: 'OnMedia Group', link: 'https://www.onmediagroup.se', time: '2018-04-23'},
        { title: this.$t('skillWebbTitle'), description: this.$t('skillWebbDescr'), text: this.$t('skillWebbText'), name: 'web', id: 'web', icon: 'fal fa-paint-brush', color: 'bg-danger', company: 'Hobby', link: '', time: '2016-08-01' },
        { title: this.$t('skillSupportTitle'), description: this.$t('skillSupportDescr'), text: this.$t('skillSupportText'), name: 'sup',  id: 'sup', icon: 'fal fa-headset', color: 'bg-primary', company: 'QuickSearch', link: 'https://www1.quicksearch.se/', time: '2016-01-15 - 2018-04-06' },
      ]
    }
  },
  computed: {
